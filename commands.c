...
int mutt_save_message(struct Header *h, bool delete, bool decode, bool decrypt)
{
...
#ifdef USE_IMAP
  if ((Context->mailbox->magic == MUTT_IMAP) && !(decode || decrypt) && (imap_path_probe(buf, NULL) == MUTT_IMAP))
  {
  ...
  if (savectx)
  {
    ...
    const bool need_mailbox_cleanup = ((savectx->mailbox->magic == MUTT_MBOX) || (savectx->mailbox->magic == MUTT_MMDF));
